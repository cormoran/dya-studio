...
| MAC-002   | `Step` で追加、Action を tap/down/up/delay/string に変更し、binding/数値を確定                              | row の Action と値が更新される。string は変換不能文字を error として表示する                                                                                                                                                        | 追加・削除では先に待機中/進行中の string/delay write を完了し、その後 step を RAM に書く。中間失敗の rollback はない                                        | S2/S4                                                                                                     |
| MAC-003   | `Reset` / `Delete`。detail 読込に失敗した場合も list から macro を選んで `Delete`                           | Reset は当該 slot の default を読込、Delete は選択中の list entry と detail 選択を消す。detail が読めなくても選択名と Delete を表示する                                                                                             | 各 mutation は memory 上の未保存変更。Delete の確認 dialog は実装されていない                                                                               | S2/S4/S6                                                                                                  |
...
- read/write RPC error は hook が `error` と loading を戻す。値は楽観更新済みである場合があり、Refresh で device の現値を取り直す。retry は Refresh、または同じ変更の再入力。macro detail の読込失敗後も list entry があれば選択名と Delete は残り、削除 RPC を試せる。
