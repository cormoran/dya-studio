...
| KM-020 | `Combos` card の横並び item / `New combo` を選ぶ             | 各 item は link icon と slot順の 1 始まり番号、name、positions、layer scope、割当を示す。RAM にのみ反映され flash 未保存の item は key switch と同じ緑の背景・文字・`Unsaved` dot を示す。保存済みで既定値と異なる item は青 dot、既定 item は dot なし。既存 item は `Combo Editor` modal を開く。+ は空 slot に既定 combo を RAM 作成して modal を開く。空きがなければ拒否を表示する。Escape/外側クリック/Close は modal を閉じる | editor の有効 draft は既存の [COMBO-001〜003](macro-combo.md) と同じ RAM 自動書込。閉じるだけでは巻き戻さない。スロット単位の未保存表示は、この画面で成功した RAM 書込みを追跡し、header Save/Discard で解除する | S1/S7、2026-09-24/2026-09-24ユーザー要求                                                    |
| KM-021 | preview の combo 表示を確認する                              | 有効で現在 layer 対象の combo のうち、辺で隣接する２キーは境界中央に通常背景の 0.5U button と左上の link icon（番号なし）を表示し、hover/focus で combo 名と割当の tooltip を表示する。その他は対象キー上部に link icon と slot順の 1 始まり番号を表示し、対象キー tooltip の key binding の下に combo 名と割当を表示する。無効/他 layer の combo は出さない                                                                        | button/badge/tooltip の閲覧だけでは書き込まない                                                                                                                                                                  | S5/S8、2026-09-24/2026-09-24ユーザー要求                                                    |
| KM-022 | preview 内の combo control を選び、behavior を変更する       | 対象 combo の `KeycodeSelector` が Combo Editor より前面に開き、`Floating mode` / `Dialog mode` で表示形式を切替できる。適用した割当が preview/card に反映される。通常の key binding は変更しない                                                                                                                                                                                                                                   | combo RAM に書く。header Save で combo flash 保存。header Discard で combo の保存済み値へ復元                                                                                                                    | S1/S7/S8、2026-09-24ユーザー要求                                                            |
...
2. 隣接２キーと離れた/3キー以上の combo がある場合、前者の通常背景 icon と後者の `<icon><N>` を照合する。隣接 control を hover/focus して combo 名・割当の tooltip を確認し、後者は対象キー tooltip の binding の下にある名前・割当を確認する。card の icon 番号・positions・layer scope と比較する。ない場合は作成後に確認する。
...
4. preview の combo control から割当を変更し、`Floating mode` / `Dialog mode` を往復しても同じ combo を編集し続けること、通常キーの binding が変わらないことを確認する。header Reset の確認に keymap と combo の対象が列挙されることを確認してCancelし、元値に戻して Save、または header Discard で復元する。
