...
| KM-023 | Runtime Macro の param1 の `New macro` / `Edit macros`       | key selector を保ったまま macro editor modal を開く。`New macro` は既存 macro を読込・変更しない作成専用 modal を開き、`Create macro` 押下時に作成 request を開始して閉じる。`Edit macros` は active binding の `param1` slot の macro を開き、作成 button を表示しない。editor を閉じると macro list を再取得し、param1 の候補と behavior dropdown の `Macro` category を最新の macro 名で表示する | modal を開閉するだけでは key binding を書かない。macro の作成・編集は Runtime Macro subsystem の keyboard memory に反映され、keymap の Save とは別 | S1/S3/S9/BIND-019、2026-09-23ユーザー要求                                                   |
