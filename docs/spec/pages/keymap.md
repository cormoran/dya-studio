...
| KM-021 | preview の combo 表示を確認する                              | 有効で現在 layer 対象の combo のうち、辺で隣接する２キーは境界中央に通常背景の約0.24Uの不透明な link icon button（番号なし）を重ねて表示し、キーラベルを隠さない。hover/focus でも背景は不透明のまま、combo 名と割当の tooltip を表示する。その他は対象キー上部に link icon と slot順の 1 始まり番号を表示し、対象キー tooltip の key binding の下に combo 名と割当を表示する。無効/他 layer の combo は出さない                                                                                                                   | button/badge/tooltip の閲覧だけでは書き込まない                                                                                                                                                                  | S5/S8、2026-09-24ユーザー要求                                                               |
...
2. 隣接２キーと離れた/3キー以上の combo がある場合、前者の小型・不透明な通常背景 icon と後者の `<icon><N>` を照合する。隣接 control を hover/focus しても下のキーが透けず、combo 名・割当の tooltip が出ることを確認し、後者は対象キー tooltip の binding の下にある名前・割当を確認する。card の icon 番号・positions・レイヤー名の独立行と比較する。card 幅を超える item が次行に折り返すことも確認する。ない場合は作成後に確認する。
