...
4. preview の combo control から `Floating mode` を開いて任意の位置へドラッグする。通常キーを開くと combo selector が閉じ、再び combo control を開くと通常キー selector が閉じ、ドラッグ済みの同じ位置に一つだけ表示されることを確認する。両方を閉じた後は既定位置へ戻る。`Dialog mode` を往復しても同じ combo を編集し続けること、通常キーの binding が変わらないことを確認する。header Reset の確認に keymap と combo の対象が列挙されることを確認してCancelし、元値に戻して Save、または header Discard で復元する。
