...
runtime sensor rotation 対応 device と current layer が必要。非対応は親で警告、対応かつ sensor 0 件では section 内のカードが空。sensor ごとに ID/name と 2 方向を表示。初期読込は全 sensor の layer bindings を取得。未接続は親の接続 gate。独立した Save/dirty 状態はない。UI は選択時リアルタイム保存と説明するが、flash 永続化の firmware 契約はこの frontend だけでは断定しない。
...
1. Demo → Keymap、section と sensor 名、選択 layer、両方向と tap time の元値を記録。
