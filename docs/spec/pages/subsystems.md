...
| S2   | [CustomSubsystemsPage](../../../src/pages/CustomSubsystemsPage.tsx): `isValidUrl`, `digestUrl`, `isUrlApproved`, `saveUrlApproval`, `ExternalLinkWarningDialog` | URL validation と localStorage approval             |
...
- approved / unapproved: http / https の legacy entry だけを移行対象とし、承認済み URL は SHA-256 digest で保持する。不正 URL は legacy storage から除外されるが、card の metadata URL が invalid のときの link click 挙動は未検証。
...
| SUB-005 | warning → Open                                | `zmkApp.disconnect()` 後に `navigateTo(url)`。checkbox ON なら URL の SHA-256 digest を保存し次回は warning を省略    | 接続を切断し外部 URL へ同一ページ navigation。digest は `dya-studio-approved-subsystem-url-digests` localStorage              | S1/S2/S4             |
| SUB-006 | approved URL を選ぶ                           | warning を出さず SUB-005 と同じ disconnect → navigation                                                               | approval entry は URL 文字列単位、firmware author / host 単位ではない。旧 plain-text entry は最初の照合時に digest へ移行する | S1/S2                |
...
- SUB-I03: URL を approve しても firmware author、同 host の別 URL、将来の URL を信頼済みにしない（S2）。
- SUB-I04: Demo checkbox だけで現在の接続が発見済み subsystem list を更新したと扱わない。再接続が反映境界である（S1/S3）。

## エラーと復帰

- localStorage または Web Crypto の失敗は catch して無視する。approval / demo override の保持失敗は UI に表示されず、次回 warning / default に戻る可能性がある。
...
2. http / https、不正 URL、approval checkbox ON/OFF、legacy entry の digest 移行、localStorage 消去後の warning を比較する。
