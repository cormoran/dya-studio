...
BIND-022: caller が modal 内から selector を開く場合、selector の overlay と content は caller modal より前面に表示する。Combo Editor の Behavior 選択では selector を背後に隠さず操作できる。Keymap preview の combo control から開く selector はこの重なり順を維持したまま modal/floating を切替でき、通常キー selector と同時に開かず同じ floating anchor とドラッグ済み位置を共有する。どちらも閉じた後は次回を既定位置から開始する。通常の caller は既存の overlay 順を使う。根拠はS1、2026-09-24ユーザー要求。
...
| BIND-022 | caller modal から combo selector を開く                       | selector の overlay/content は caller の前面。Keymap の combo と通常キー selector は同時に開かず、floating は同一 anchor とドラッグ位置を共有する。両方を閉じると既定位置へ戻す | 表示切替・閲覧だけでは binding を送らない                                   | S1/BIND-022                |
