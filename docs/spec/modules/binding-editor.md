...
BIND-019: `Runtime Macro`、`Trans`、`None` は behavior dropdown の `Key Press` category に表示する。keyboard が Runtime Macro behavior を公開し、登録済み macro が一つ以上ある場合は、各 macro を `Macro` category の個別項目として表示する。個別項目を選ぶと Runtime Macro behavior とその slot を `param1` にした binding を caller に渡す。Runtime Macro の param1 候補の下には `New macro` と `Edit macros` を右寄せで同じ行に置く。`New macro` は名前・step を入力できる作成専用 editor を開き、作成前の入力と Close は device を変更しない。`Create macro` 成功時にだけ RAM に作成して閉じ、候補を更新する。`Edit macros` は active binding の `param1` slot を開き、`Create macro` を表示しない。編集の debounce 状態は待機中・書込み中・RAM 反映済みを示し、`Save and close` は queued write を flush して macro を flash に保存した後に閉じる。根拠はS1/S4、2026-09-24ユーザー要求。

BIND-020: `Edit macros` の modal を閉じる際、今回の modal 内で名前または step を変更した場合は確認 dialog を表示する。`Keep editing` では変更を維持して編集へ戻る。`Discard edits and close` では queued write を cancel し、進行中の書込み完了を待ってから、modal を開いた時点の同じ slot の名前と step に RAM を戻し、成功後に閉じる。開く前から RAM にあった未保存変更は残す。変更がなければ確認なしで閉じる。reset/delete は slot を維持した復元が保証できないため、この modal では表示しない。復元 RPC が失敗した場合は modal を開いたまま error を示して再試行できる。`discardMacros` は全 macro の未保存変更を失うため使わない。根拠は [MacroEditorDialog](../../../src/components/macroCombo/MacroEditorDialog.tsx)、[restoreMacroMemory](../../../src/components/macroCombo/restoreMacroMemory.ts)、2026-09-24ユーザー要求。
...
| BIND-019 | Runtime Macro の param1 候補で `New macro` / `Edit macros` を選ぶ、または `Macro` category の登録済み macro を選ぶ | 両 button は候補の下で右寄せの同じ行に表示。`New macro` は名前・step の未保存 draft を表示し、Close で破棄。`Create macro` 成功時に RAM へ作成して閉じる。`Edit macros` は active binding の `param1` slot を開き、作成 button を表示しない。編集中は `Waiting to apply...` → `Applying to memory...` → `Applied to memory` を表示し、`Save and close` 成功後に閉じる。macro list と param1 候補を最新にする。登録済み macro を選ぶと Runtime Macro + 選んだ slot を完成 binding として扱う | 作成前の draft/Close は device 無変更。作成・自動反映は RAM、`Save and close` は flash。個別 macro の選択は通常の parameter 完了規則に従う | S1/S4/BIND-R05             |
| BIND-020 | `Edit macros` で値を変更し X / Escape / 外側クリックを試す                                                         | 変更時は `Discard macro edits?` を表示。`Keep editing` は復帰、`Discard edits and close` 成功後は modal が閉じる。変更なしは確認なしで閉じる。復元失敗時は error を示して開いたまま                                                                                                                                                                                                                                                                                                         | 対象 slot の開いた時点の RAM 値へ戻す。開く前の未保存値と他 macro の RAM 値を保持し、flash へ保存しない                                    | BIND-020                   |
...
behavior ごとの firmware validation、ストレージ拒否、全 consumer での失敗復帰は未検証。modifier clear が自動適用を誘発する挙動は現行実装の記録であり、望ましさの承認は確認していない。BIND-020 の個別 slot 復元は名前・step の RAM 値を戻すが、firmware は個別 slot の未保存フラグを取り消す RPC を提供しないため、値が元通りでも `Unsaved` 表示が残り得る。全体の Discard で消すと modal を開く前の RAM-only 変更も失うため行わない。
