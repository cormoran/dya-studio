...
| HIST-011 | Trackball restore                   | runtime processors（対応時は慣性 / Fast input を含む）と PMW3610 custom settings を一 confirm で差分適用。消えた processor/setting は skip。旧snapshotの慣性省略は変更なし、旧deviceへの慣性restoreはskip | processor RPC は persistent write-through。custom settings は memory write で section Save が必要 | S5       |
