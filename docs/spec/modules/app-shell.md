...
- SHELL-013: viewport が横向きかつ幅1023px以下・高さ500px以下のとき、固定高shell内のpage単位縦scrollを解除し、header・tab列・main contentを含むdocument全体を縦scrollする。これにより低いモバイル横画面でheaderを画面外へ移動し、main contentへ高さを使える。条件外では従来どおりheader/tab列を固定したまま各pageが縦scrollし、tabごとのscroll位置を維持する。横overflow、保存、tab保持の契約は変更しない。根拠: ユーザー要求（2026-09-20）、S1/S3/S4、[共通CSS](../../../src/index.css)。
