...
- Orca 環境では利用可能な `orca-cli` / `orchestration` skill に従う。worker にも **orca command は sandbox 外（`exec_command` の `require_escalated`）で実行**と明記する。昇格が許可されない場合は制約を報告し、回避しない。
