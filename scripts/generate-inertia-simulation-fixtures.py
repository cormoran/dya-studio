...
cases.append(('scroll-scale', 300, 20, 10, 8, 0, 15, 300, 1, 60, 2400))
fixtures = []
for (name, *config) in cases:
    reports = []
    rem = 0
    (m, d) = config[7:9]
    peak = config[9] if len(config) > 9 else 40
    for time in range(20, 5000, 20):
        raw = int(peak * (1 - ((time - 3200) / 3200) ** 2) + 0.5)
        num = raw * m + rem
        scaled = num // d
        rem = num - scaled * d
